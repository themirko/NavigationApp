...
void KDTree::buildTreeRecursive(KDPtr& currentRoot,
  ...
  if (depth % 2 == 0)
    std::sort(nodes.begin(), nodes.end(), compareByLatitude);
  else
    std::sort(nodes.begin(), nodes.end(), compareByLongitude);
...
std::pair<nodePtr, Kilometers> KDTree::findNearestNodeRecursive(
  const Degrees latitude,
  const Degrees longitude,
  const KDPtr& currentRoot,
  const long depth,
  std::pair<nodePtr, Kilometers> currentBestNode) {
  if (currentRoot == nullptr) return currentBestNode;

  Kilometers currentDistance =
    GeoUtils::HaversineDistance(latitude, longitude, currentRoot->parent);

  std::pair<nodePtr, Kilometers> bestNode =
    std::make_pair(currentRoot->parent, currentDistance);

  if (currentBestNode.first &&
      currentBestNode.second < currentDistance) {
  ...
  if (!(depth % 2)) {
    ...
    targetAxis = latitude;
    axisDistance =
      GeoUtils::HaversineDistance(parentAxis, 0, targetAxis, 0);
  } else {
    parentAxis = currentRoot->parent->longitude;
    targetAxis = longitude;
    axisDistance =
      GeoUtils::HaversineDistance(0, parentAxis, 0, targetAxis);
  }

  const KDPtr exploreCloser =
    parentAxis < targetAxis ? currentRoot->right : currentRoot->left;
  const KDPtr exploreFurther =
    parentAxis < targetAxis ? currentRoot->left : currentRoot->right;

  bestNode = findNearestNodeRecursive(latitude,
                                      longitude,
                                      exploreCloser,
                                      depth + 1,
                                      bestNode);
...
nodePtr KDTree::findNearestNode(const Degrees latitude,
                                const Degrees longitude) {
  return findNearestNodeRecursive(latitude, longitude,
                                  this->root, 0, {}).first;
}
